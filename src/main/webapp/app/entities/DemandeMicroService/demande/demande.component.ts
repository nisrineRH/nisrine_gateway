...
export class DemandeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.demandeService
            ...
            .subscribe(
                (res: HttpResponse<IDemande[]>) => this.paginateDemandes(res.body, res.headers),
    ...
    trackId(index: number, item: IDemande) {
        return item.id;
    }

    registerChangeInDemandes() {
        this.eventSubscriber = this.eventManager.subscribe('demandeListModification', response => this.loadAll());
    }
